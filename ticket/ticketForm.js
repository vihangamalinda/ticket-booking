...
mainSubmitBtn.addEventListener("click",(e)=>{
    ...
    // const arrayObj = Array.from(ticketList);
    // console.log(arrayObj);

    // for(i=0;i<ticketList.childNodes.length)
    // ticketList.childNodes.map((node,index)=>{
    //     node.remove();
    // })
    debugger
    // ticketList.remove();
    console.log(ticketList.childNodes);
    debugger
...
const ticketsPoper = (addedTicketArray) =>{
    for(i=0; i< addedTicketArray.length ;i++){
        addedTicketArray.pop()
    }
    const ticketList = document.getElementsByClassName("selected-ticket-list")[0];
    console.log(ticketList.childNodes);
}
